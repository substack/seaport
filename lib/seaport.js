...
function Seaport (opts) {
    ...
    function lookupKey () {
        var spub = String(opts.public.data);
        var keys = Object.keys(self._authorized);
        for (var i = 0; i < keys.length; i++) {
            if (self._authorized[keys[i]].data === spub) {
                return keys[i];
            }
        }
        keys = Object.keys(self.authorized.rows);
        for (var i = 0; i < keys.length; i++) {
            if (self.authorized.rows[keys[i]].state.key.data === spub) {
                return keys[i];
            }
        }
    }
    ...
    self.doc = !opts.private || !opts.public ? new crdt.Doc : new(crdt.Doc)({
        ...
        sign : function (update) {
            if (opts.private) {
                var s = crypto.createSign(opts.private.algorithm);
                s.update(JSON.stringify(update.slice(0,3)));
                var x = s.sign(opts.private.data, 'base64');
        ...
        verify : function (update, cb) {
            ...
            var v = crypto.createVerify(key.algorithm)
                .update(JSON.stringify(update.slice(0,3)))
                .verify(key.data, sig[1], 'base64')
            ;
            cb(null, v);
