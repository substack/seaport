...
function Seaport (opts) {
    ...
    if (!opts) opts = {};
    opts.heartbeatInterval = parseInt(opts.heartbeatInterval) || 15 * 1000;
    self._authorized = {};
    ...
    self.doc.on('row_update', function (row) {
        ...
        if (row.state.type === 'service' && row.state._heartbeat) {
            var offset = Date.now() - (row.state._heartbeat || 0);
            self._timeOffsets[row.id] = offset;
        }
        
        if (row.state._node === self.doc.id
        && row.state.type === 'stale-service') {
            row.set('type', 'service');
            self.emit('reclaim', row);
        }
    });
    ...
    function heartbeatChecker() {
        ...
        self.services.forEach(function (row) {
            var t = (row.get('_heartbeat') || 0) - self._timeOffsets[row.id];
            if (t < staleTime) {
                row.set('type', 'stale-service');
                self.emit('stale', row);
            }
        });
        self._heartbeatCheckerTimer = setTimeout(
            heartbeatChecker, opts.heartbeatInterval * 2
        );
    }
...
Seaport.prototype.registerMeta = function (role, opts) {
    ...
    var id = meta.id = opts.id || createId();
