...
Seaport.prototype.createStream = function (addr) {
    ...
    function onend () {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            self.emit('unregister', self.known[key]);
        }
    }
};
...
Seaport.prototype.register = function (meta) {
    ...
    for (var i = 0; i < this.endpoints.length; i++) {
        registerServices(mserv, this.endpoints[i]);
    }
...
Seaport.prototype.query = function (meta) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        for (var j = 0; j < mkeys.length; j++) {
            var mkey = mkeys[j];
            if (mkey === 'version') {
                if (!semver.satisfies(row.version, meta.version)) break;
            }
            else if (row[mkey] !== meta[mkey]) break;
        }
        if (j === mkeys.length) rows.push(row);
    }
    return rows;
};
...
Protocol.prototype._write = function (row, enc, next) {
    ...
    else if (row[0] === 'unregister') {
        var meta = this.known[row[1]];
        delete this.known[row[1]];
        this.emit('unregister', row[1], meta);
    }
    next();
};
